...
function random_input()
  ...
  global ReRate_MNO=[16,16,16,16] #request rate of each user
  N_UE=sum(N_UE_s,1)[1]
  UE = zeros(2,N_UE)
  ...
  rate_vt=[ReRate_MNO[1]*ones(N_UE_s[1],1); ReRate_MNO[2]*ones(N_UE_s[2],1); ReRate_MNO[3]*ones(N_UE_s[3],1); ReRate_MNO[4]*ones(N_UE_s[4],1)]
  ...
  # index=[1,21,40,59,77]
  index= [1, N_UE_s[1]+1, sum(N_UE_s[1:2])+1, sum(N_UE_s[1:3])+1,sum(N_UE_s)+1]
